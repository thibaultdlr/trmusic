    ...
    if ($obj->IsAuthenticated == true && !is_user_logged_in() && !is_admin()) {
      ...
      if (!empty($lrdata['Email']) OR (empty($lrdata['Email']) && $dummyemail == 'dummyemail')) {
        ...
        if (!empty($wp_user_id)) {
          ...
          if ( isset( $LoginRadius_settings['LoginRadius_socialLinking'] ) && ( $LoginRadius_settings['LoginRadius_socialLinking'] == 1 ) ) {
...
function LoginRadius_admin_menu() {
  $page = @add_menu_page('LoginRadius', '<b style="color:#0ccdfe;">Login</b><b style="color:#000;">Radius</b>', 8, 'LoginRadius', 'LoginRadius_option_page');
